...
Initial provisioning of Linux boxes is left outside the scope of this project. We assume a working CentOS 7, Debian 9 or Ubuntu 18.04 system. If you are installing this repo on the box itself, check out the repo in to a local directory, verify the variables in `vars/common.yml`, then do the following:
...
|`bundler_version` | The version of the gem bundler to install. |
...
|`hyrax_smtp_address` | Rails smtp address, defaults to smpt.example.com |
|`hyrax_smtp_port` | Rails smtp port, defaults to 25 |
|`hyrax_smtp_domain` | Rails smtp domain for mailer |
|`hyrax_smtp_user_name` | Rails smtp username for mailer | 
|`hyrax_smtp_password` | Rails smtp password for mailer |
|`hyrax_smtp_authentication` | Rails authentication method for mailer | 
|`hyrax_smtp_enable_starttls_auto` | Rails starttls setting for mailer |
|`hyrax_smtp_openssl_verify_mode` | Rails openssl verify mode for mailer |
|`hyrax_smtp_ssl` | Rails ssl setting for mailer | 
|`hyrax_smtp_tls` | Rails tls setting for mailer |
